...
namespace com.krafton.fantasysports.UI
{
    ...
    public class PanelSwitchHandler : MonoBehaviour
    {
            ...
            if (!isSwitching)
                {
                    ...
                    switch (switchBehavior)
                    {
                        ...
                        StartCoroutine(WipeScreen(0.1f, panel, isHidden, true));
                            break;
                        case SwitchType.wipeRight:
                        StartCoroutine(WipeScreen(0.1f, panel, isHidden, false));
                            break;
                    }
        ...
        private IEnumerator WipeScreen(float timeToComplete, RectTransform target, bool hide, bool wipeLeft)
        {
            ...
            {
                currentTime += Time.deltaTime;
                lerpAmount = currentTime / timeToComplete;

                target.anchoredPosition = Vector2.Lerp(target.anchoredPosition, endPos, lerpCurve.Evaluate(lerpAmount));
