...
namespace Gpp.CommonUI
{
    internal static class GppUI
    {
        ...
        static GppUI ()
        {
            ...
            {
                GppPrefabs.Add(UIType.Toast, new GppUIProperty("GppCommonUI/Pc/GppPcToastUI", true));
            }
            else if (PlatformUtil.IsMobile())
            {
                GppPrefabs.Add(UIType.Toast, new GppUIProperty("GppCommonUI/Console/GppConsoleToastUI", true));
            }
            else
            {
                GppPrefabs.Add(UIType.Toast, new GppUIProperty("GppCommonUI/Common/GppToastUI", true));
            }
        }
        ...
        public static void ShowRepayRequired(List<RefundRestriction> refundRestrictionList, List<RefundRestriction> purchasedRefundList, List<IapProduct> iapProductList, Action<RefundRestriction> onSuccess, Action<GameObject> onFail)
        {
            ShowUI(UIType.RepayRequired, ui =>
            {
                ...
                uiComponent.SetRestrictionProductList(refundRestrictionList, purchasedRefundList, iapProductList);
            });
        }

        public static void ShowRepayRequiredError(string key, Action<GameObject> onFail)
        {
            ShowUI(UIType.RepayRequired, ui =>
            {
                var uiComponent = ui.GetComponent<IGppRepay>();
                uiComponent.DrawCommonErrorMessage(key, onFail);
            });
        }
