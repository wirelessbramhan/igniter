...
namespace Gpp.Auth
{
    internal static class GppAuth
    {
                ...
                if (result.IsError)
                {
                    ...
                    switch (errorCode)
                    {
                        ...
                        case ErrorCode.RepayRequired:
                            FunnelLog.Purchase.RepaymentFunnel.RepayUserEntry();
                            OpenRepayUI((result) =>
                            {
                                if (result.IsError)
                                {
        ...
        public static void GetKIDUserInfoWebURL(KIDUserInfoUri userInfoUri, ResultCallback<KIDUserInfoWebURLData> callback)
        {
            if (!GppSDK.IsLoggedIn)
                callback.TryError(ErrorCode.NotLoggedIn);
            else if (!GppSDK.GetSession().cachedTokenData.IsFullKid)
                callback.TryError(ErrorCode.NotFullKID);
            else
                GppSDK.GetIamApi().GetKIDUserInfoWebURL(userInfoUri, callback);
        }
        ...
        {
            FunnelLog.Purchase.RepaymentFunnel.TriggerPurchaseRecovery();
            GppSDK.GetUnconsumedProducts(result =>
            {
                if (!result.IsError)
                {
                    FunnelLog.Purchase.RepaymentFunnel.RequestRepayList();
                    GppSDK.GetPlatformApi().GetRestrictions((restrictionProducts) =>
                    {
                        ...
                        else
                        {
                            ...
                            {
                                GppRepayManager gppRepayManager = new GppRepayManager(restrictionProducts.Value,
                                    () =>
                                    {
