...
namespace Gpp.Api.Platform
{
    internal partial class PlatformApi
    {
        ...
        private IEnumerator RequestVerifyStoreReceipt(StoreReceipt receipt, string platform, string funnelType, ResultCallback<VerifiedReceipt> callback)
        {
            var request = HttpRequestBuilder
            ...
            request.Headers.Add("X-Payment-Funnel-ID", GppTokenProvider.CreateGuestUserId());
            request.Headers.Add("X-Payment-Funnel-Type", funnelType);
        ...
        private IEnumerator RequestVerifyStoreReceipts(List<StoreReceipt> receipts, string platform, string funnelType, ResultCallback<Dictionary<string, VerifiedReceipt>> callback)
        {
            Dictionary<string, VerifiedReceipt> verifiedReceipts = new Dictionary<string, VerifiedReceipt>();
            foreach (var receipt in receipts)
            {
                yield return RequestVerifyStoreReceipt(receipt, platform, funnelType, result =>
                    {
                        if (!result.IsError)
                        {
