...
namespace Gpp.Api
{
    internal partial class IamApi
    {
        ...
        private const string IamOAuthRedirectKIDUserInfoWeb = "/oauth2/internal/ns/{namespace}/p/krafton/redirect/web";
        ...
        private IEnumerator RequestKIDUserInfoWebURL(KIDUserInfoUri redirectUri, ResultCallback<KIDUserInfoWebURLData> callback)
        {
            Dictionary<string, object> body = new Dictionary<string, object>
            {
                { "locale", Session.PlayerLocale },
            };
            if (redirectUri != KIDUserInfoUri.None)
            {
                string bodyUri = string.Concat("/v2/settings/", redirectUri.ToString());
                body.Add("redirect_uri", bodyUri);
            }

            var request = HttpRequestBuilder.CreatePost(ServiceUrl + IamOAuthRedirectKIDUserInfoWeb)
                .WithBearerAuth(Session.AccessToken)
                .WithPathParam("namespace", Namespace)
                .WithBody(body.ToJsonString())
                .WithContentType(MediaType.ApplicationJson)
            ...
            yield return HttpClient.SendRequest(request, (response, _) =>
            {
                ...
                var result = response.TryParseJson<KIDUserInfoWebURLData>();
                callback.Try(result);
            }
            );
        }
